...
# Virgin SSB
ssb0 = format(model$derived_quants[model$derived_quants$Label == "SSB_Virgin", 2], digits = 2, scientific = FALSE, big.mark = ",") 

# Spawning Biomass at 25%
ssb_targ = round(0.25 * model$derived_quants[model$derived_quants$Label == "SSB_Virgin", 2], 0)

# Final Year SSB
ssb_2021 = format(model$derived_quants[model$derived_quants$Label == paste0("SSB_",endyr), 2], digits = 0, scientific = FALSE, big.mark = ",") 
...
start = which(model$derived_quants$Label == paste0("Bratio_",startyr))
...
end = which(model$derived_quants$Label == paste0("SPRratio_2020",endyr-1))
all = model$derived_quants[start:end, c("Label","Value")]
...
# Exploitation
start = which(model$derived_quants$Label == paste0("F_1892",startyr))
