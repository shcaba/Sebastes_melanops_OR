...
end = which(model$derived_quants$Label == paste0("SPRratio_",endyr-10))
all = model$derived_quants[start:end, c("Label","Value")]
...
grab = which(all$Label == paste0("SPRratio_",endyr-1))
spr_recent_10 = round(all[grab:nrow(all), "Value"],2)

# Exploitation
start = which(model$derived_quants$Label == paste0("F_",startyr))
end = which(model$derived_quants$Label == paste0("F_",endyr-10))
all = model$derived_quants[start:end, c("Label","Value")]
grab = which(all$Label == paste0("F_",endyr-11))
