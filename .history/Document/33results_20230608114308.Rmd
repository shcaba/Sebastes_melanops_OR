...
The two fishery independent surveys, the MPA (Figure \ref{fig:mpa-index-fit}) and the tagging survey (Figure \ref{fig:tag-index-fit}) are also not well fit. 

to the ORBS recreational survey index are generally poor as  a population increasing in the early 2000s then leveling off and dropping over recent years (). The decreasing trend is more consistent over the last decade than indicated in the lengths or ages. Inital variance for the survey is extremely small, though it only took a small amount of added variance to fit the index (`r round(model$parameters[grep("Q_extraSD_Recreational",model$parameters$Label),]$Value, 2)`). The catchability coefficient (q) `r round(exp(model$parameters[model$parameters$Label == "LnQ_base_Recreational(2)", 3]), 3)` was analytically solved for and very small relative to the total an absoute measure (q=1), a typical result of a fishery-based abundance index.
